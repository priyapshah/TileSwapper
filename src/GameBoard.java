...
public class GameBoard extends JPanel {
	...
	public void fillBoard() { //adds tiles to board
	...
	public void newGameBoard() { //resets
		clearBoard();
		fillBoard();
		resetScore();
		counter = 0;
	...
	public boolean checkGroup(int x, int y) {
		//adds matching tiles to HashSet
	...
	public void removeChains() { 
		//Use of collections was discussed and approved of following game proposal feedback
	...
	public void moveDown() { //drops above tiles down to fill in removed match
		for (int i = 0; i < NUM_ROW_COL; i++) {
			for (int j = 0; j < NUM_ROW_COL; j++) {
				if (board[i][j] != null) {
					if (j + 1 < NUM_ROW_COL) {
	...
	public void clearBoard() { //clears entire board of tiles
		for (int i = 0; i < NUM_ROW_COL; i++) {
			for (int j = 0; j < NUM_ROW_COL; j++) {
				if (board[i][j] != null) {
					board[i][j] = null;
